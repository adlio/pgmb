...
// FindReleaseEvents retrieves a slice of ReleaseEvent which match the
// supplied criteria.
//
func FindReleaseEvents(db DB, clauses ...QueryFunc) (events []*ReleaseEvent, err error) {
	events = make([]*ReleaseEvent, 0)
	err = Select(db, &events, ReleaseEventQuery(), clauses...)
	return
}
...
func ReleaseEventMap(db DB, releaseIDs []int64) (events map[int64][]*ReleaseEvent, err error) {
	...
	results, err := FindReleaseEvents(db, Where("release IN (?)", releaseIDs))
...
// ReleaseEventQuery builds the default query for the release_event view
//
func ReleaseEventQuery() sq.SelectBuilder {
